...
public:
    ...
    explicit CCircularBuffer(const size_type capacity) :
    ...
    CCircularBuffer(const std::initializer_list<T>& list) :
        ...
        for (CCircularBuffer::iterator it = begin(); it != end(); ++it) {
            std::allocator_traits<alloc>::construct(allocator, &(*it), *iter);
            ++iter;
        }
    }

    CCircularBuffer(value_type k, size_type amount) :
    capacity_(amount + 1),
    ...
    CCircularBuffer(const CCircularBuffer& other) {
        ...
        for (CCircularBuffer::iterator i = head_; i != tail_; ++i) {
    ...
    CCircularBuffer& operator=(const CCircularBuffer& other) {
        ...
        tail_ = head_ + size_;
        iterator j = other.head_;
        for (CCircularBuffer::iterator i = begin(); i != end(); ++i) {
            std::allocator_traits<alloc>::construct(allocator, &(*i), *j);
            ++j;
        }
        return *this;
    ...
    constexpr bool operator==(CCircularBuffer& other) {
        if (size_ != other.size_) {
            return false;
        } else {
            iterator j = other.begin();
            for (CCircularBuffer::const_iterator i = cbegin(); i < cend(); ++i) {
                if (*i != *j) {
                    return false;
                }
                ++j;
    ...
    constexpr bool operator!=(const CCircularBuffer& other) const noexcept {
        return !(*this == other);
    }
    ...
    static void swap(CCircularBuffer& first, CCircularBuffer& second) {
...
public:
    ...
    explicit CCircularBufferExt(const size_type capacity) :
            capacity_(capacity + 1),
            buff_(std::allocator_traits<alloc>::allocate(allocator, capacity)),
            head_(Iter<value_type>(buff_, capacity + 1)),
            tail_(++Iter<value_type>(buff_, capacity + 1)) {}

    CCircularBufferExt(const std::initializer_list<T>& list) :
            ...
            tail_(head_ + size_) {
        auto iter = list.begin();
        for (CCircularBufferExt::iterator it = begin(); it != end(); ++it) {
            std::allocator_traits<alloc>::construct(allocator, &(*it), *iter);
            ++iter;
        }
    }

    CCircularBufferExt(value_type k, size_type amount) :
    ...
    CCircularBufferExt(iterator it1, iterator it2) {
    ...
    CCircularBufferExt(const CCircularBufferExt& other) {
        ...
        tail_ = head_ + size_;
        iterator j = other.head_;
        for (CCircularBufferExt::iterator i = begin(); i != end(); ++i) {
            std::allocator_traits<alloc>::construct(allocator, &(*i), *j);
            ++j;
        }
    }
    ...
    constexpr bool operator==(CCircularBufferExt& other) {
        if (size_ != other.size_) {
            return false;
        } else {
            iterator j = other.begin();
            for (CCircularBufferExt::const_iterator i = cbegin(); i < cend(); ++i) {
                if (*i != *j) {
                    return false;
                }
                ++j;
    ...
    constexpr bool operator!=(const CCircularBufferExt& other) {
        return !(*this == other);
    }
    ...
    constexpr void swap(CCircularBufferExt& other) {
        std::swap(capacity_, other.capacity_);
        std::swap(size_, other.size_);
        std::swap(head_, other.head_);
        std::swap(tail_, other.tail_);
    ...
    constexpr static void swap(CCircularBufferExt& first, CCircularBufferExt& second) {
        std::swap(first.capacity_, second.capacity_);
        std::swap(first.size_, second.size_);
        std::swap(first.head_, second.head_);
        std::swap(first.tail_, second.tail_);
